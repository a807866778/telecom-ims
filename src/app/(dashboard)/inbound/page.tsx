  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <Link href="/inbound/new" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
          + 新建入库单
        </Link>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b">
                ...
                <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">入库时间</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {records.map((record) => (
                <tr key={record.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm">
                    <Link href={`/inbound/${record.id}`} className="text-blue-600 hover:text-blue-700">
                      查看详情
                    </Link>
                  </td>
                </tr>
              ...
              {records.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center text-gray-400">暂无入库记录</td>
                </tr>
              )}
            </tbody>
          </table>
