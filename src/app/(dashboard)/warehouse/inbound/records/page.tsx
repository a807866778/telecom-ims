...
const TYPE_COLORS: Record<string, string> = {
  "直接入库": "bg-blue-100 text-blue-700",
  "采购入库": "bg-green-100 text-green-700",
  "报溢入库": "bg-yellow-100 text-yellow-700",
...
export default function InboundRecordsPage() {
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/warehouse/inbound/records?${params}`);
  ...
  const handleExport = () => {
    ...
    window.open(`/api/warehouse/inbound/records/export?${params}`, "_blank");
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h1 className="text-xl font-semibold text-gray-900">入库记录</h1>
        <div className="flex gap-2">
          ...
          <Link href="/inbound" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
            新建入库单
          </Link>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-4">
        <div className="flex gap-4 flex-wrap items-end">
          <div>
            <label className="block text-sm text-gray-600 mb-1">入库类型</label>
            <select
              value={typeFilter}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#1e3a5f] text-white">
                <th className="px-4 py-3 text-left text-sm font-medium">入库单号</th>
                    ...
                    <tr className="bg-gray-50">
                      <td colSpan={8} className="px-4 py-4">
                        <div className="space-y-3">
                          ...
                          {/* 入库明细 */}
                          <div className="text-sm font-medium text-gray-700">入库明细：</div>
                          ...
                          {record.photoUrl && (
                            <div className="mt-3">
                              <div className="text-sm font-medium text-gray-700 mb-2">入库照片：</div>
                              <img
                                src={record.photoUrl}
                                alt="入库照片"
